...
class TestBoards:
    ...
    def test_board_successful_creation(self, board_setup):
        assert board.create_board_response.status_code == 200, logger.exception('Assertion Failed')
        assert board.create_board_response.json()['name'] == first_board_name, logger.exception('Assertion Failed')
    ...
    def test_successful_board_update(self, board_setup):
        ...
        board.update_board_details(base_url=base_url, url_for_board=url_for_board, board_id=boardId,
                                   querystring=querystring_for_board)
        assert board.update_board_response.status_code == 200, logger.exception('Assertion Failed')
        assert board.update_board_response.json()['name'] == board_updated_name, logger.exception('Assertion Failed')
